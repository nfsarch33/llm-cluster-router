// Package deploy (v18716.6) validates the Quadlet distribution
// at deploy/helixchannel/quadlet/. The tests do NOT require
// podman or systemd to be installed — they parse the unit files
// and assert the canonical Quadlet structure. A separate TDD
// step (running `podman build` on a real host) gates deployment;
// the local runner path can stop at parser validation.
//
// This file lives under deploy/helixchannel/ rather than
// internal/ because it tests deploy-time artefacts that are not
// part of any Go package import graph. `go test ./deploy/...`
// discovers it without an internal helper package.
