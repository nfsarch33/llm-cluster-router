...
func TestRealModelE2E_LightsailSmoke(t *testing.T) {
	...
	if err != nil {
		...
		if strings.Contains(errStr, "non-200 response") && (strings.Contains(errStr, "401") || strings.Contains(errStr, "403")) {
			t.Skipf("upstream rejected the credentials (key revoked / model retired). v18710-3 architecture verified; key refresh required — operator action: rotate 1Password item <vault-name>/<uuid> (Aliyun Team Qwen Token Plan Key). err=%v", err)
		}
